...
class SparseTwoCliqueAttentionLayer(nn.Module):
    ...
    def forward(self, x, edge_index, d0_index, twoClique_index, d1_index):
        ...
        # NOW HANDLE ALL EDGES (there is some redundancy here)
        # Compute products [nEdges, heads, att_dim']
        q_i = q[edge_index[0, :], :, :]
        k_j = k[edge_index[1, :], :, :]
        if not self.opt['no_symmetrize']:
        ...
        if self.opt['attention_type'] == 'scaled_dot':
            if self.opt['no_symmetrize']:
                ...
                in_ij = 0.5*(q_i * k_j + q_j * k_i)
            pre = torch.sum(in_ij, dim=-1)
            ...
            if self.opt['no_symmetrize']:
                ...
                in_ij = 0.5*(q_i - k_j + q_j - k_i)
            arg = torch.sum((in_ij)**2, dim=1) / (2*self.lengthscale**2)
            pre = self.output_var**2 * torch.exp(-arg)
            ...
            if self.opt['no_symmetrize']:
                ...
                pre = 0.5*(cos(q_i, k_j) + cos(q_j, k_i))
        ...
        # Top degree is always f(pre)
        if self.opt['use_squareplus']:
            diagA1full = squareplus(pre)
        else:
            diagA1full = torch.exp(pre)
        ...
        '''replace zeros from edges contained in no 2-clique'''
        torch.where(diagA1 > 0, diagA1, diagA1full)
