...
class ODEBlock(nn.Module):
    ...
    def set_x0(self, x0):
        if not self.opt['2clique_features']:
            q0, p0 = x0
...
class ODEFunc(nn.Module):
    def __init__(self, opt, data):
        ...
        if opt['bracket'] == 'metriplectic_old':
            # For convenience
    ...
    def forward(self, t, x):
        ...
        if self.opt['bracket'] == 'hamiltonian':
            ...
            qPart = -alpha * spmm(self.d0t_index, self.d0t_vals,
                                  *self.d0_shape[::-1], A1 * p) / A0
            pPart =  alpha * spmm(self.d0_index, self.d0_vals, 
                                  *self.d0_shape, q)
            if self.opt['2clique_features']:
                pPart2 = -alpha * spmm(self.d1t_index, self.d1t_vals,
                                       *self.d1_shape[::-1], A2 * r) / A1
                rPart  = alpha * spmm(self.d1_index, self.d1_vals, 
                                      *self.d1_shape, p)
                pPart  = pPart + pPart2
            ...
            if self.opt['bracket'] == 'gradient_q_only':
                ...
                if self.opt['2clique_features']:
                    tmp   = spmm(self.d1t_index, self.d1t_vals,
                                *self.d1_shape[::-1], A2 * r) / A1
                    rPart = - alpha * spmm(self.d1_index, self.d1_vals,
                                           *self.d1_shape, tmp)
            ...
            if self.opt['2clique_features']:
                rPart1 = spmm(self.d1_index, self.d1_vals,
                              *self.d1_shape, p)
                pPart3 = spmm(self.d1t_index, self.d1t_vals,
                              *self.d1_shape[::-1], A2 * rPart1) / A1
                pPart4 = spmm(self.d1t_index, self.d1t_vals,
                              *self.d1_shape[::-1], A2 * r) / A1
                rPart2 = spmm(self.d1_index, self.d1_vals,
                              *self.d1_shape, pPart4)
                pPart = pPart - pPart3 - pPart4
                rPart = alpha * rPart1 - rPart2

        elif self.opt['bracket'] == 'metriplectic_old':
            # Metriplectic system (NOT implemented for 2-cliques)
            ...
            # L part of equations to solve
            qPartL = -spmm(self.d0t_index, self.d0t_vals,
                           *self.d0_shape[::-1], A1 * p) / A0
            pPartL =  spmm(self.d0_index, self.d0_vals, 
                           *self.d0_shape, q)
            
            # M part of equations to solve
            normdE2 = torch.sum( A0 * q * q ) + torch.sum( A1 * p * p )
            d1p     = spmm(self.d1_index, self.d1_vals, 
                           *self.d1_shape, p)
            delEtdS = beta * torch.sum( A0 * q ) + torch.sum( A2 * d1p )

            d1s1    = spmm(self.d1t_index, self.d1t_vals, 
                           *self.d1_shape[::-1], A2 * torch.ones_like(d1p)) / A1
            qPartM  = beta * torch.ones_like(q) - delEtdS * q / normdE2
            pPartM  = d1s1 - delEtdS * p / normdE2

            qPart   = alpha * (qPartL + qPartM)
            pPart   = alpha * (pPartL + pPartM)

        elif self.opt['bracket'] == 'node':
            # TODO: implement for 2-clique features
        ...
        if self.opt['verbose']:
            if self.opt['bracket'] != 'metriplectic_old':
                E = 0.5 * (torch.sum(A0*q**2) + torch.sum(A1*p**2))
