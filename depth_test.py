...
def main(opt):
    ...
    # return train_acc, val_acc, test_acc, time.time()-whole_time

    dicts = torch.load(model_name)
    params, opts = dicts['model_state_dict'], dicts['options_dict']
    model.load_state_dict(params)
    model.odeblock.register_buffer('tRange', 
                                   torch.tensor([0, opt['final_time']]))

    start_time = time.time()
    train_acc, val_acc, test_acc = tester(model, data, opts)
    end_time = time.time() - start_time

    log = 'Runtime {:03f}, Train Acc {:03f}, Valid Acc {:03f},' \
          ' Test Acc {:03f}'
    print(log.format(end_time, train_acc, val_acc, test_acc))            

    return train_acc, val_acc, test_acc, end_time
...
if __name__ == '__main__':
    ...
    train_acc = torch.zeros(n_runs, len(timeList))
    valid_acc = torch.zeros(n_runs, len(timeList))
    test_acc = torch.zeros(n_runs, len(timeList))
    runtime = torch.zeros(n_runs, len(timeList))
    ...
    for n in range(n_runs):
        for i,t in enumerate(timeList):
            options = vars(args)
            options['final_time'] = t
            # print(options['final_time'], options['step_size'])
            train_acc[n,i], valid_acc[n,i], test_acc[n,i], runtime[n,i] = main(options)

    print('constant step size experiment')
    print(f'train accs/stds are {torch.std_mean(train_acc,0)}')
    print(f'valid accs are {torch.std_mean(valid_acc,0)}')
    print(f'test accs are {torch.std_mean(test_acc,0)}')
    print(f'runtimes are {torch.std_mean(runtime, 0)}') 
